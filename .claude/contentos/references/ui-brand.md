...
Visual patterns for user-facing CONTENT:OS output. All commands @-reference this file.

## Brand Color

CONTENT:OS brand color is **teal/emerald** (ANSI 43).
- Use `\033[38;5;43m` to set teal text, `\033[0m` to reset
- Apply teal to: the CONTENT:OS block-letter banner, mode headers (`<< CONTENT:OS // MODE >>`), section titles in dashboards
...
Display once when CONTENT:OS loads. Render the block letters in teal.
...
Use for workflow transitions. CONTENT:OS uses angle brackets.

```
<< CONTENT:OS // {MODE NAME} >>
```
...
- `/content:publish` — pushing live content to any platform
...
## Anti-Patterns — What CONTENT:OS never does
